...
For AutoGrader (student app):
    1. Create assignments.xlsx (using Assignment Editor or create-assignments-excel.py)
    2. Configure config.ini with email settings
    3. Run: python encode_resources.py
    4. Then run this build script
"""
...
def clean_build_folders():
    ...
    for f in os.listdir('.'):
        if f.endswith('.spec') and f not in ['assignment-editor.spec', 'autograder_build.spec']:
            print(f"  Removing {f}")
            os.remove(f)
...
def build_autograder(debug=False):
    ...
    elif os.path.exists('pyinstaller-spec-method2.py'):
        # Rename to .spec
        spec_file = 'autograder.spec'
        shutil.copy('pyinstaller-spec-method2.py', spec_file)
        print("Using pyinstaller-spec-method2.py as spec file")
    else:
        print("❌ No spec file found!")
        print("   Either use Assignment Editor to generate one, or ensure")
        print("   pyinstaller-spec-method2.py is in the current directory.")
        return False
